...
class Critter: NSObject, NSCoding  {
    ...
    static let ArchiveURL =
        DocumentsDirectory?.appendingPathComponent("critters",
            isDirectory: false)
    //MARK: Types
    struct PropertyKey {
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        let photo = aDecoder.decodeObject(forKey: PropertyKey.photo)
            as? UIImage
        let details = aDecoder.decodeObject(forKey: PropertyKey.details)
            as? String
